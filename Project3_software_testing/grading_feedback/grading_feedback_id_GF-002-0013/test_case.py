...
import time
from constant import COURSE_NAME, ASSIGNMENT_NAME, LONG_FEEDBACK
...
class Test_grading(unittest.TestCase):
    ...
    def test_edit_grade(self):
        ...
        input_feedback.send_keys(LONG_FEEDBACK)
        time.sleep(2)

        input_fullname = driver.find_element(By.NAME, "grade")
        input_fullname.clear()
        input_fullname.send_keys("70.21")
        button_save = driver.find_element(By.NAME, "savechanges")
        button_save.click()

        try:
            # Adjust the max wait time as needed
            WebDriverWait(driver, 10).until(
                EC.visibility_of_element_located((By.CLASS_NAME, "toast-message"))
            )
            print("Pop-up is visible - Test Passed")
        except TimeoutException:
            print("Pop-up did not appear in time - Test Failed")

        # self.assertTrue("The changes to the grade and feedback were saved" in self.driver.page_source)
