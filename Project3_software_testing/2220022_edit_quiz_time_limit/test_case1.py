...
def edit_quiz(self):
  ...
  time.sleep(1)
  time_limit.clear()
  time_limit.send_keys("-1")

  print('{:67s}{:5s}{:4s}'.format("7.  Set time limit to -1","  -  ","Done"))
  ...
  # Confirm the input time limit -1 is NOT accepted - Begin
  print('{:67s}{:5s}{:4s}'.format("9.  Confirm the input time limit -1 is NOT accepted","  -  ","Done"))
  print("\nThe time limit input -1 is still accepted BUT the result is set to 1\n")
  assert "Grading method: Highest grade" not in self.driver.page_source
  # Confirm the input time limit -1 is NOT accepted - End
...
class MoodleTest(unittest.TestCase):
  ...
  def test_EDIT_QUIZ_1(self):
    ...
    7.  Set time limit to -1
    8.  Click 'Save and display'
    9.  Confirm the input time limit -1 is NOT accepted
    """
