...
def view_quiz_result(self):
  ...
  # In 'Attemps that are: Uncheck all choices - Begin
  ...
  for attemp in attemps_id:
    ...
    if choice.is_selected(): choice.click()

  print('{:78s}{:5s}{:4s}'.format("6.  In 'Attemps that are: Uncheck all choices","  -  ","Done"))
  # In 'Attemps that are: Uncheck all choices - End
  ...
  time.sleep(2)
  assert "You must select at least one state." in self.driver.page_source
...
class MoodleTest(unittest.TestCase):
  ...
  def test_VIEW_QUIZ_RESULT_5(self):
    print('\n')
    print('----------------------------------------------------------------------------------------------------------------------')
    ...
    5.  Click 'Results'
    6.  In 'Attemps that are: Uncheck all choices
